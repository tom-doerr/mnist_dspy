...
class MNISTEnsemble:
    """Manages ensemble of classifiers with majority voting and hard example tracking"""
    ...
    def evaluate(self) -> float:
        ...
        test_data = MNISTData().get_training_data()  # Use training data for boosting
        def ensemble_predict(pixel_matrix: str) -> dspy.Prediction:
            predictions = []
            for clf in self.classifiers:
                result = clf(pixel_matrix=pixel_matrix)
                predictions.append(str(result.number).strip())  # Clean whitespace from predictions
